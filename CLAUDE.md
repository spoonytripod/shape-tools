...
- 배포는 GitHub Pages(`main` 브랜치, 저장소 루트). live demo URL은 `README.md` 참고.
...
### 도구 모듈 패턴 (핵심)
- 각 도구는 독립 IIFE다: `initPyramid()`, `initArrows()`. 자체 상태(`layerCount`, `arrowCount`, `thickness` 등), 자체 `PRESETS`, `buildSvg()`, `render()`, 이벤트 핸들러를 모두 캡슐화한다.
- 같은 DOM을 공유하므로 **DOM id는 도구별 접두사로 네임스페이스 분리**한다: 피라미드 `pyr-*`, 화살표 `arr-*`. 지역 `$ = id => document.getElementById(id)`로 참조한다.
