...
새 Next.js 프로젝트에는 의존성과 실행 명령을 관리할 도구가 필요했다. 결정 당시 저장소에는 `package.json`이나 잠금 파일이 없고, 로컬 환경에는 Node와 npm이 설치되어 있었다.
...
이 결정은 npm과 파일 관리 규칙을 확정했다. 설치된 버전은 `package-lock.json`에 기록되며, 테스트·린트·포맷 도구는 [0004](0004-quality-tooling.md)에서 정했다.
