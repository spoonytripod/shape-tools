...
패키지 관리자는 [0003](0003-package-manager.md)에서 npm으로 정했다. 테스트·린트·포맷 도구는 [0004](0004-quality-tooling.md)에서 정했다. URL 구조는 [0005](0005-url-structure.md)에서 정했다. 정확한 프레임워크·도구 버전과 첫 신규 도형은 아직 결정하지 않았다. Vercel 프로젝트·도메인 설정은 전환 후 배포 단계에서 정한다. Next.js의 정적 내보내기 사용 여부도 현재는 정하지 않는다. 구체적 필요가 생길 때 결정 기록에 근거를 남긴다.
